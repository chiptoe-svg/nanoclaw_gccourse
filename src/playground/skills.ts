...
import { CONTAINER_SKILLS_DIR, DRAFT_SKILLS_DIR } from './paths.js';
...
export function readSkill(
  name: string,
): { content: string; origin: SkillSummary['origin']; supportingFiles: string[] } | null {
  ...
  const lookIn = origin === 'draft' || origin === 'overlay' ? DRAFT_SKILLS_DIR : CONTAINER_SKILLS_DIR;
...
/**
 * Delete a skill from the draft overlay only. Never touches the shared
 * container/skills directory — students can always re-apply by editing
 * the shared copy.
 */
export function deleteDraftSkill(name: string): void {
  if (!isValidSkillName(name)) throw new Error(`Invalid skill name: ${name}`);
  const draftDir = path.join(DRAFT_SKILLS_DIR, name);
