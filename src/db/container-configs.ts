  ...
  getDb()
    .prepare(
      `INSERT INTO container_configs (
        ...
        additional_mounts, cli_scope, env, allowed_models, model_provider, updated_at
      ) VALUES (
        @agent_group_id, @provider, @model, @effort, @image_tag, @assistant_name,
        @max_messages_per_prompt, @skills, @mcp_servers, @packages_apt, @packages_npm,
        @additional_mounts, @cli_scope, @env, @allowed_models, @model_provider, @updated_at
...
/** Overwrite a JSON column wholesale. Used for skills, mcp_servers, packages_*, additional_mounts, env, allowed_models. */
export function updateContainerConfigJson(
  agentGroupId: string,
  column: 'skills' | 'mcp_servers' | 'packages_apt' | 'packages_npm' | 'additional_mounts' | 'env' | 'allowed_models',
  value: unknown,
): void {
