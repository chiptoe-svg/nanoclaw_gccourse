...
describe('handleApplyProviderToClass (C-2 instructor card Apply button)', () => {
  beforeEach(() => {
    // The handler validates specIds against listProviderSpecs(). The spec
    // modules self-register at import time via model-catalog.js side-effects
    // — but vi.resetModules() in the outer beforeEach (and module isolation
    // in vitest) means we can't rely on registration order across describe
    // blocks. Mock the registry directly so the test stays focused on the
    // handler logic, not on import-side-effect ordering.
  ...
  it('sets allow + provideDefault on each registered specId, leaves allowByo alone', async () => {
    fs.writeFileSync(
      path.join(tmpRoot, 'config', 'class-controls.json'),
      JSON.stringify({
        classes: {
          default: {
            ...
            authModesAvailable: [],
            providers: {
              codex: { allow: false, provideDefault: false, allowByo: true },
            },
          },
        },
      }),
  ...
  it('rejects unknown specIds with 400', async () => {
    const { handleApplyProviderToClass } = await import('./class-controls.js');
    const r = handleApplyProviderToClass({ specIds: ['not-a-real-spec'] });
    expect(r.status).toBe(400);
  });

  it('rejects empty specIds with 400', async () => {
    const { handleApplyProviderToClass } = await import('./class-controls.js');
    const r = handleApplyProviderToClass({ specIds: [] });
    expect(r.status).toBe(400);
  });

  it('quietly ignores unknown ids when mixed with known ones', async () => {
    const { handleApplyProviderToClass, readClassControls } = await import('./class-controls.js');
    const r = handleApplyProviderToClass({ specIds: ['claude', 'not-a-real-spec'] });
    expect(r.status).toBe(200);
    expect(readClassControls().classes.default!.providers.claude!.allow).toBe(true);
  });
});
